...
//    for (InfoType *type in self.titleArray) {
//        CGFloat width =  [type.typeName sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
//        frame.size.width = MAX(width, frame.size.width);
//    }
    for (NSString *typeName in self.titleArray) {
        CGFloat width =  [typeName sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
        frame.size.width = MAX(width, frame.size.width);
    }
...
//    InfoType *infoType = [self.titleArray objectAtIndex:[indexPath row]];
//    self.titleArr = [[NSMutableArray alloc]init];
    NSString *typeName = [self.titleArray objectAtIndex:[indexPath row]];
//    NSLog(@"%@",infoType.typeName);
    cell.textLabel.text = typeName;
    NSLog(@"%@",self.titleName);
    if ([typeName isEqualToString:self.titleName]) {
        cell.textLabel.textColor =[UIColor colorWithRed:140 green:199 blue:63 alpha:1];
    }
    [self.titleArr addObject:typeName];
    NSLog(@"%@",self.titleArr);
    return cell;
}
...
//        NSDictionary *dictData = [NSDictionary dictionaryWithObject:infoType.typeName forKey:@"typeName"];
//        //定义广播
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"infoTypeName" //广播名称
